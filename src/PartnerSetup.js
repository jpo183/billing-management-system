...
const PartnerSetup = () => {
  ...
  const fetchPartners = () => {
    fetch(`http://localhost:5050/api/partners?showInactive=${showInactive}`)
  ...
  const handleSave = () => {
    ...
    const url = editingPartner
      ? `http://localhost:5050/api/partners/${editingPartner.id}`
      : "http://localhost:5050/api/partners";
    ...
    fetch(url, {
      ...
      .catch((error) => console.error("Error saving partner:", error));
  };
